...
- **`useInvite().acceptInvite()` surfaces `invite_email_mismatch` when the invite was sent to a different email address (BUG-AUTH-10, 2026-05-31).** When the accounts-service returns its 403 `FORBIDDEN` "Invite email does not match authenticated user" response, the hook sets `error = { code: 'invite_email_mismatch', message: 'This invitation was sent to a different email address. Please sign in with the invited account.' }` instead of falling through to the generic `unknown_error`. The BUG-AUTH-4 refresh-token recovery path is intentionally skipped for this case — the backend has explicitly rejected the join, so a workspace-list lookup must NOT falsely "confirm" success. Consumers can branch on `error.code === 'invite_email_mismatch'` to render an actionable "sign in with the correct account" CTA.
